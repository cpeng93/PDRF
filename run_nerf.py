...
def config_parser():
    ...
    parser.add_argument("--kernel_type", type=str, default='kernel',
                        help='choose among <none>, <DSK>, <PBE>')
...
def train():
    ...
    if args.dataset_type == 'llff':
        ...
        print('Loaded llff', images.shape, render_poses.shape, hwf, args.datadir, bounding_box)
    ...
    if args.kernel_type == 'PBE' or args.kernel_type == 'DSK':
        kernelnet = BlurModel(len(images), torch.tensor(poses[:, :3, :4]),
    ...
    if args.mode == 'c2f':
        optimizer = torch.optim.Adam(params=[{'params': nerf.grad_vars, 'lr': 0.001}, 
                                            {'params': nerf.grad_vars_vol, 'lr': 0.02}],
                                     betas=(0.9, 0.999))
    elif args.mode == 'nerf':
        ...
        with torch.no_grad():
            ...
            if args.render_test:
                for rgb_idx, rgb8 in enumerate(rgbs):
                    ...
                    imageio.imwrite(os.path.join(testsavedir, f'{rgb_idx:03d}_disp.png'), cv2.applyColorMap(255-curr_disp, cv2.COLORMAP_TWILIGHT_SHIFTED))
            else:
    ...
    for i in range(start, N_iters):
        ...
        if "TV" in extra_loss:
            loss = loss + extra_loss["TV"]
        if "align" in extra_loss:
            if vars(args)["align_start_iter"] <= i <= vars(args)["align_end_iter"]:
                loss = loss + extra_loss["align"] * vars(args)["kernel_align_weight"]
